...
const CallToAction: React.FC<Props> = (props) => {
    ...
    return (
        <div className='fixed bottom-0 left-0 z-30 dark:bg-darkBlue bg-white drop-shadow-[0_35px_35px_rgba(0,0,0,0.2)] w-full h-20 px-4 flex justify-between items-center font-medium'>
            ...
            {getSubmitBtn === 'delete' ?
                <button  type="submit" className='w-full mx-1 dark:bg-red-500 bg-red-500/90 dark:hover:bg-red-700 hover:bg-red-400 h-[60%] rounded-full duration-300 ease-in-out text-[.95rem] text-white'>{submitBtn}</button>
                :
            ...
            {props.markPaid && targetedInvoice.status === 'pending' &&
                <button onClick={markPaidHandler} type="submit" className={`w-full mx-1 dark:bg-primaryPurple bg-indigo-600 dark:hover:bg-indigo-600 hover:bg-primaryPurple h-[60%] rounded-full duration-300 ease-in-out text-[.95rem] text-white`}>{props.markPaid}</button>
            }
        </div>
    )
}
