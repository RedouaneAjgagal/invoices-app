...
const CallToAction: React.FC<Props> = (props) => {
    ...
    return (
        <div className='fixed bottom-0 left-0 z-30 dark:bg-darkBlue bg-white drop-shadow-[0_35px_35px_rgba(0,0,0,0.2)] w-full h-20 px-4 flex justify-between items-center font-medium'>
            <button type='button' onClick={secondaryBtnAction} className={`w-full h-[60%] mx-1 dark:bg-slate-600/10 bg-slate-200 hover:bg-slate-300 rounded-full hover:dark:bg-white hover:dark:text-primaryPurple duration-300 ease-in-out text-[.95rem]`}>{secondaryBtn}</button>
            {props.buttons.length === 3 && <button name='middleBtn' value={'Save as Draft'} className={`w-full m-1 h-[60%] bg-gray-700 text-white rounded-full hover:bg-gray-600 duration-300 ease-in-out text-[.95rem]`}>Save as Draft</button>}
            {getSubmitBtn === 'delete' ?
